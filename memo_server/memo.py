...
def connect_db():
    conn = pymysql.connect(
        host="43.203.5.205", user="root", password="pass", db="memo", charset="utf8"
    )
...
def onOAuthAuthorizationCodeRedirected():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'users' AND TABLE_SCHEMA = 'memo');"
        )
        ...
        if exists == 1:
            ...
            cur.execute(
                f"INSERT IGNORE INTO users (id, name) VALUES ('{user_id}', '{user_name}');"
            )
            cur.execute("SELECT * FROM users;")
            print(f"유저 db생성 완료, 현재 users 테이블 목록 \n{cur.fetchall()}\n")
        else:
            cur.execute(
                "CREATE TABLE users (id VARCHAR(255) PRIMARY KEY, name VARCHAR(255));"
            )
            cur.execute(
                "CREATE TABLE memos (id INT AUTO_INCREMENT PRIMARY KEY, text VARCHAR(255), user_id VARCHAR(255), FOREIGN KEY (user_id) REFERENCES users(id));"
            )
            cur.execute(
                "ALTER TABLE users MODIFY name VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_bin;"
            )
            cur.execute(
                "ALTER TABLE memos MODIFY text VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_bin;"
            )
            cur.execute(
